defmodule LightGrid do
  ...
  defp handle_light(grid, "turn on", pos) do
    Map.update(grid, pos, 1, fn v -> v + 1 end)
  end
  defp handle_light(grid, "turn off", pos) do 
    Map.update(grid, pos, 0, fn v -> max(0, v - 1) end)
  end
  defp handle_light(grid, "toggle", pos) do
    Map.update(grid, pos, 2, fn v -> v + 2 end)
  end
end
